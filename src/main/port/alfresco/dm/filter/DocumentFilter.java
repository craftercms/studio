...
public class DocumentFilter extends AbstractFilter {
    ...
    public boolean filter(DmContentItemTO item) {
        boolean isMatched = match(item.getContentType());
        boolean isDocument = (isMatched && !item.isComponent());
        item.setDocument(isDocument);      //it may have been set already, but still resetting.
        return isDocument;
    }
}
