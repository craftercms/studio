...
public abstract class PluginUtils {
    ...
    public static void validatePluginParameters(final Plugin plugin, final Map<String, String> params)
        ...
        if (CollectionUtils.isEmpty(plugin.getParameters())) {
            logger.debug1("There are no parameters defined for plugin: {}", plugin.getId());
            return;
        }

        for(Parameter param : plugin.getParameters()) {
            logger.debug1("Checking parameter {} for blueprint {}", param.getName(), plugin.getId());
            if (param.isRequired()) {
        ...
        logger.debug1("All required parameters are present for blueprint: {}", plugin.getId());
    }
