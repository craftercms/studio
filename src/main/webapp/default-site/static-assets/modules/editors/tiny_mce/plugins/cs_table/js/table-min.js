...
if(!tinymce.isIE){q.innerHTML='<br data-mce-bogus="1"/>'
...
m+=">";
if(v){if(!tinymce.isIE){m+='<caption><br data-mce-bogus="1"/></caption>'
...
for(var i=0;
i<l;
i++){if(!tinymce.isIE){m+='<td><br data-mce-bogus="1"/></td>'
}else{m+="<td></td>"
}}m+="</tr>"
}m+="</table>";
...
}tinymce.each(t.select("table[data-mce-new]"),function(y){var A=t.select("td",y);
try{d.selection.select(A[0],true);
d.selection.collapse()
}catch(x){}t.setAttrib(y,"data-mce-new","")
});
d.addVisual();
d.execCommand("mceEndUndoLevel");
tinyMCEPopup.close()
