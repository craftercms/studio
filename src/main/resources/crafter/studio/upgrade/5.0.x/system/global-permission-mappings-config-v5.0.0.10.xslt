...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
	...
	<xsl:template match="role[@name='system_admin']/rule/allowed-permissions">
		<xsl:copy>
			<xsl:copy-of select="@*"/>
			<xsl:apply-templates/>
			<xsl:if test="not(permission = 'system_properties_read')">
				<xsl:element name="permission">
					<xsl:text>system_properties_read</xsl:text>
				</xsl:element>
				<xsl:text>&#10;</xsl:text>
			</xsl:if>
			<xsl:if test="not(permission = 'system_properties_manage')">
				<xsl:element name="permission">
					<xsl:text>system_properties_manage</xsl:text>
				</xsl:element>
				<xsl:text>&#10;</xsl:text>
			</xsl:if>
		</xsl:copy>
