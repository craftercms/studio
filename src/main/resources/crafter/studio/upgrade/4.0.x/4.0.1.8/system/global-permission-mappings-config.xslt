...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
    ...
    <xsl:template match="permissions/role[@name='system_admin']/rule[@regex='/.*']/allowed-permissions">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:apply-templates/>
            <xsl:if test="not(permission = 'view_log_levels')">
                <xsl:element name="permission">
                    <xsl:text>view_log_levels</xsl:text>
                </xsl:element>
                <xsl:text>&#10;</xsl:text>
            </xsl:if>
            <xsl:if test="not(permission = 'configure_log_levels')">
                <xsl:element name="permission">
                    <xsl:text>configure_log_levels</xsl:text>
                </xsl:element>
                <xsl:text>&#10;</xsl:text>
            </xsl:if>
        </xsl:copy>
