...
v_parent_id BIGINT;
    DECLARE
v_parent_path VARCHAR(2000);
    DECLARE
v_parent_item_path VARCHAR(2000);
    DECLARE
v_finished INTEGER DEFAULT 0;
    DECLARE
parent_cursor CURSOR FOR
SELECT i.id                              as parent_id,
       REPLACE(i.path, '/index.xml', '') AS parent_path,
       i.path
                                         AS parent_item_path
FROM item i
WHERE i.site_id = siteId
  AND (path = rootPath OR path LIKE concat(rootPath, '/%'));
DECLARE
...
: LOOP
        ...
        IF
v_finished = 1 THEN LEAVE update_parent;
END IF;
UPDATE item
