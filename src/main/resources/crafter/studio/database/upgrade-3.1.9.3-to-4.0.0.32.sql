...
UPDATE site
SET last_synced_gitlog_commit_id = last_commit_id;
...
(
    ...
    INDEX `cluster_remote_ix_remote_id`
(
    `remote_repository_id`
    ASC
),
...
(
    ...
    `node_last_commit_id` VARCHAR
(
    50
) NULL DEFAULT NULL,
    `node_last_verified_gitlog_commit_id` VARCHAR
(
    50
) NULL DEFAULT NULL,
    `node_last_synced_gitlog_commit_id` VARCHAR
(
    50
) NULL DEFAULT NULL,
    `site_state` VARCHAR
(
    ...
    `site_published_repo_created` INT
(
    11
) NOT NULL DEFAULT 0,
    PRIMARY KEY
(
    ...
    INDEX `cluster_site_ix_remote_id`
(
    `site_id`
    ASC
),
...
BEGIN
    DECLARE
v_lock_owner_id VARCHAR(255);
    DECLARE
v_lock_heartbeat DATETIME;
SELECT publishing_lock_owner, publishing_lock_heartbeat
INTO v_lock_owner_id, v_lock_heartbeat
FROM site
...
locked = 0;
    IF
(v_lock_owner_id IS NULL OR v_lock_owner_id = '' OR v_lock_owner_id = lockOwnerId OR DATE_ADD(v_lock_heartbeat, INTERVAL ttl MINUTE) < CURRENT_TIMESTAMP)
    THEN
UPDATE site
SET publishing_lock_owner     = lockOwnerId,
    publishing_lock_heartbeat = CURRENT_TIMESTAMP
WHERE site_id = siteId
  AND deleted = 0;
SET
locked = 1;
