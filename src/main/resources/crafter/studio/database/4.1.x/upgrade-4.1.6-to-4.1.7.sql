...
SELECT null,
       ...
       1,
       'ready',
       s.last_verified_gitlog_commit_id,
       IFNULL(sandboxBranch, 'master'),
       s.published_repo_created,
       s.publishing_lock_owner,
       s.publishing_lock_heartbeat,
       'INITIALIZING',
       s.last_synced_gitlog_commit_id
FROM site s
...
SELECT null, siteId, d.source_path, d.target_path, d.type
FROM dependency d
WHERE d.site = sourceSiteId;

INSERT INTO gitlog (id, site_id, commit_id, processed, audited)
SELECT null, siteId, gl.commit_id, gl.processed, gl.audited
FROM gitlog gl
WHERE gl.site_id = sourceSiteId;
