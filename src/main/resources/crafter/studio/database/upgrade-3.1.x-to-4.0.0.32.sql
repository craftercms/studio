...
DROP
COLUMN `publishing_status_message`,
DROP
COLUMN `status` ;
...
(
    ...
    INDEX `user_property_ix_site_id`
(
    `site_id`
    ASC
),
...
(
    ...
    INDEX `item_ix_created_by`
(
    `created_by`
    ASC
),
    INDEX `item_ix_last_modified_by`
(
    `last_modified_by`
    ASC
),
    INDEX `item_ix_locked_by`
(
    `locked_by`
    ASC
),
    INDEX `item_ix_parent`
(
    `parent_id`
    ASC
),
...
(
    ...
    INDEX `item_translation_ix_source`
(
    `source_id`
    ASC
),
    INDEX `item_translation_ix_translation`
(
    `translation_id`
    ASC
),
...
(
    ...
    INDEX `workflow_ix_item`
(
    `item_id`
    ASC
),
    INDEX `workflow_ix_submitter`
(
    `submitter_id`
    ASC
),
    INDEX `workflow_ix_reviewer`
(
    `reviewer_id`
    ASC
),
...
(
    20
) NULL DEFAULT NULL AUTO_INCREMENT,
    `user_id` BIGINT
(
    20
) NULL DEFAULT NULL,
...
INSERT
IGNORE INTO `user` (record_last_updated, username, password, first_name, last_name,
...
FROM
    (SELECT im.path as item_path, ist.state as item_state, ist.system_processing as item_sys_process, im.lockowner as item_owner, im.creator as item_creator, im.modifier as item_modifier, im.modified as item_modified, im.commit_id as item_commit_id, submittedtoenvironment as item_wf_env, IFNULL((SELECT if(state = 'COMPLETED' AND environment = 'live', 1, 0)
    FROM publish_request WHERE site = siteId and path = item_path ORDER BY scheduleddate DESC LIMIT 1), 0) as live, IFNULL((SELECT if(state = 'COMPLETED' AND environment = 'staging', 1, 0)
    FROM publish_request WHERE site = siteId and path = item_path ORDER BY scheduleddate DESC LIMIT 1), 0) as staging, IF(submittedtoenvironment = 'live', 1, 0) AS dest
    FROM item_state ist
...
v_renamed INT;
    DECLARE
v_previous_path VARCHAR(2048);
    DECLARE
v_state VARCHAR(255);
    DECLARE
v_state_str VARCHAR(255);
    DECLARE
v_submitter VARCHAR(255);
    DECLARE
v_submitter_id BIGINT;
    DECLARE
v_submitter_comment TEXT;
    DECLARE
v_reviewer VARCHAR(255);
    DECLARE
v_reviewer_id BIGINT;
    DECLARE
v_reviewer_comment TEXT;
    DECLARE
v_schedule TIMESTAMP;
    DECLARE
v_notify_submitter INT;
    DECLARE
v_last_modified_by BIGINT;
    DECLARE
v_last_modified_on TIMESTAMP;
    DECLARE
v_target_environment VARCHAR(255);
    DECLARE
v_publishing_package_id VARCHAR(50);

    DECLARE
v_finished INTEGER DEFAULT 0;
    DECLARE
item_cursor CURSOR FOR
SELECT ist.path                  as item_path,
       ist.state                 as item_state,
       im.renamed                as item_renamed,
       im.oldurl                 as item_previous_path,
       im.submittedtoenvironment as item_target_environment,
       im.submittedby            as item_submitter,
       im.submissioncomment      as item_submitter_comment,
       im.launchdate             as item_scheduled,
       im.sendemail              as item_notify_submitter
FROM item_state ist
         LEFT OUTER JOIN item_metadata im ON ist.site = im.site AND ist.path = im.path
...
v_previous_path = NULL;
        SET
v_target_environment = NULL;
        SET
v_submitter = NULL;
        SET
v_submitter_comment = NULL;
        SET
v_schedule = NULL;
        SET
v_notify_submitter = 0;
        SET
v_site_id = NULL;
        SET
v_item_id = NULL;
...
v_reviewer_id = NULL;
        SET
v_reviewer_comment = NULL;
        SET
v_publishing_package_id = NULL;
...
IF
v_notify_submitter IS NULL THEN
...
CASE
            WHEN v_state = 'NEW_SUBMITTED_WITH_WF_SCHEDULED' THEN
SELECT 'OPENED'
INTO v_state_str;
WHEN v_state = 'NEW_SUBMITTED_WITH_WF_SCHEDULED_LOCKED' THEN
SELECT 'OPENED'
INTO v_state_str;
WHEN v_state = 'NEW_SUBMITTED_WITH_WF_UNSCHEDULED' THEN
SELECT 'OPENED'
INTO v_state_str;
WHEN v_state = 'NEW_SUBMITTED_WITH_WF_UNSCHEDULED_LOCKED' THEN
SELECT 'OPENED'
INTO v_state_str;
WHEN v_state = 'NEW_SUBMITTED_NO_WF_SCHEDULED' THEN
SELECT 'APPROVED'
INTO v_state_str;
WHEN v_state = 'NEW_SUBMITTED_NO_WF_SCHEDULED_LOCKED' THEN
SELECT 'APPROVED'
INTO v_state_str;
WHEN v_state = 'NEW_SUBMITTED_NO_WF_UNSCHEDULED' THEN
SELECT 'APPROVED'
INTO v_state_str;
WHEN v_state = 'EXISTING_SUBMITTED_WITH_WF_SCHEDULED' THEN
SELECT 'OPENED'
INTO v_state_str;
WHEN v_state = 'EXISTING_SUBMITTED_WITH_WF_SCHEDULED_LOCKED' THEN
SELECT 'OPENED'
INTO v_state_str;
WHEN v_state = 'EXISTING_SUBMITTED_WITH_WF_UNSCHEDULED' THEN
SELECT 'OPENED'
INTO v_state_str;
WHEN v_state = 'EXISTING_SUBMITTED_WITH_WF_UNSCHEDULED_LOCKED' THEN
SELECT 'OPENED'
INTO v_state_str;
WHEN v_state = 'EXISTING_SUBMITTED_NO_WF_SCHEDULED' THEN
SELECT 'APPROVED'
INTO v_state_str;
WHEN v_state = 'EXISTING_SUBMITTED_NO_WF_SCHEDULED_LOCKED' THEN
SELECT 'APPROVED'
INTO v_state_str;
WHEN v_state = 'EXISTING_SUBMITTED_NO_WF_UNSCHEDULED' THEN
SELECT 'APPROVED'
INTO v_state_str;
...
FROM (SELECT id
      FROM user
      WHERE username = v_submitter
      UNION
      SELECT id
      FROM user
      WHERE username = v_submitter LIMIT 1) as a;

SELECT a.id
INTO v_reviewer_id
FROM (SELECT id
      FROM user
      WHERE username = v_reviewer
      UNION
      SELECT id
      FROM user
      WHERE username = v_reviewer LIMIT 1) as a;

SELECT submissioncomment, package_id
INTO v_reviewer_comment, v_publishing_package_id
FROM publish_request
WHERE state = 'READY_FOR_LIVE'
  AND site = siteId
  and path = v_path
...
v_renamed > 0 THEN
UPDATE item
SET previous_path = v_previous_path
WHERE id = v_item_id;
END IF;

        IF
v_state_str IS NOT NULL THEN
