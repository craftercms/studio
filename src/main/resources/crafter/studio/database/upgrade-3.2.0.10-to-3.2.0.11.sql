...
v_created_on TIMESTAMP;
    DECLARE
v_modifier VARCHAR(255);
    DECLARE
v_last_modified_by BIGINT;
    DECLARE
v_last_modified_on TIMESTAMP;
    DECLARE
v_commit_id VARCHAR(128);
    DECLARE
v_finished INTEGER DEFAULT 0;
    DECLARE
item_cursor CURSOR FOR
SELECT im.path               as item_path,
       ist.state             as item_state,
       ist.system_processing as item_sys_process,
       im.owner              as item_owner,
       im.creator            as item_creator,
       im.modifier           as item_modifier,
       im.modified           as item_modified,
       im.commit_id          as item_commit_id
FROM item_state ist
...
SELECT a.id
INTO v_owned_by
FROM (SELECT id FROM user WHERE username = v_owner UNION SELECT id from user WHERE username = v_owner LIMIT 1) as a;
SELECT a.id
INTO v_created_by
FROM (SELECT id FROM user WHERE username = v_creator UNION SELECT id from user WHERE username = v_creator LIMIT 1) as a;
SELECT a.id
INTO v_last_modified_by
FROM (SELECT id FROM user WHERE username = v_modifier UNION SELECT id from user WHERE username = v_modifier LIMIT 1) as a;
