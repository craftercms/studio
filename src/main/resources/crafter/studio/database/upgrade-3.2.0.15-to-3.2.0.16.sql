...
UPDATE cluster_site_sync_repo cssr INNER JOIN site s
ON cssr.site_id = s.id SET cssr.site_state = 'CREATED'
WHERE s.deleted = 0;

UPDATE cluster_site_sync_repo cssr INNER JOIN site s
ON cssr.site_id = s.id SET cssr.site_state = 'DELETED'
WHERE s.deleted = 1;

UPDATE cluster_site_sync_repo cssr INNER JOIN site s
ON cssr.site_id = s.id SET cssr.site_published_repo_created = s.published_repo_created;

UPDATE _meta
SET version = '3.2.0.16';
