...
(function(){var c=YAHOO.util.Dom,a=YAHOO.util.Event,b=YAHOO.lang,d=YAHOO.widget.Toolbar;
YAHOO.widget.Editor=function(e,f){YAHOO.widget.Editor.superclass.constructor.call(this,e,f)
};
...
},_undoNodeChange:function(){var f=this.toolbar.getButtonByValue("undo"),e=this.toolbar.getButtonByValue("redo");
if(f&&e){if(this._hasUndoLevel()){this.toolbar.enableButton(f)
}if(this._undoLevel<this._undoCache.length){this.toolbar.enableButton(e)
}}},_checkUndo:function(){var g=this._undoCache.length,e=[];
if(g>=this.get("maxUndo")){for(var f=(g-this.get("maxUndo"));
f<g;
f++){e.push(this._undoCache[f])
}this._undoCache=e
...
YAHOO.widget.Editor.superclass.init.call(this,e,f)
...
for(var h=0;
h<e.length;
h++){if(e[h].getAttribute("href",2)){f=e[h].getAttribute("src",2);
if(this._isLocalFile(f)){c.addClass(e[h],this.CLASS_LOCAL_FILE)
}else{c.removeClass(e[h],this.CLASS_LOCAL_FILE)
}}}var g=this._getDoc().body.getElementsByTagName("a");
for(var i=0;
i<g.length;
i++){if(g[i].getAttribute("href",2)){f=g[i].getAttribute("href",2);
if(this._isLocalFile(f)){c.addClass(g[i],this.CLASS_LOCAL_FILE)
}else{c.removeClass(g[i],this.CLASS_LOCAL_FILE)
}}}},_disabled:["createlink","forecolor","backcolor","fontname","fontsize","superscript","subscript","removeformat","heading","indent"],_alwaysDisabled:{outdent:true},_alwaysEnabled:{hiddenelements:true},_handleKeyDown:function(f){YAHOO.widget.Editor.superclass._handleKeyDown.call(this,f);
var g=false,e=null,h=false;
switch(f.keyCode){case this._keyMap.JUSTIFY_LEFT.key:if(this._checkKey(this._keyMap.JUSTIFY_LEFT,f)){e="justifyleft";
g=true
}break;
case this._keyMap.JUSTIFY_CENTER.key:if(this._checkKey(this._keyMap.JUSTIFY_CENTER,f)){e="justifycenter";
g=true
}break;
case 221:case this._keyMap.JUSTIFY_RIGHT.key:if(this._checkKey(this._keyMap.JUSTIFY_RIGHT,f)){e="justifyright";
g=true
}break
}if(g&&e){this.execCommand(e,null);
a.stopEvent(f);
this.nodeChange()
}},_renderCreateLinkWindow:function(){var e='<label for="'+this.get("id")+'_createlink_url"><strong>'+this.STR_LINK_URL+':</strong> <input type="text" name="'+this.get("id")+'_createlink_url" id="'+this.get("id")+'_createlink_url" value=""></label>';
e+='<label for="'+this.get("id")+'_createlink_target"><strong>&nbsp;</strong><input type="checkbox" name="'+this.get("id")+'_createlink_target" id="'+this.get("id")+'_createlink_target" value="_blank" class="createlink_target"> '+this.STR_LINK_NEW_WINDOW+"</label>";
e+='<label for="'+this.get("id")+'_createlink_title"><strong>'+this.STR_LINK_TITLE+':</strong> <input type="text" name="'+this.get("id")+'_createlink_title" id="'+this.get("id")+'_createlink_title" value=""></label>';
var h=document.createElement("div");
h.innerHTML=e;
var f=document.createElement("div");
f.className="removeLink";
var g=document.createElement("a");
g.href="#";
g.innerHTML=this.STR_LINK_PROP_REMOVE;
g.title=this.STR_LINK_PROP_REMOVE;
a.on(g,"click",function(i){a.stopEvent(i);
...
this._windows.createlink.body=h;
h.style.display="none";
this.get("panel").editor_form.appendChild(h);
this.fireEvent("windowCreateLinkRender",{type:"windowCreateLinkRender",panel:this.get("panel"),body:h});
return h
...
}}this.on("afterExecCommand",function(){var g=new YAHOO.widget.EditorWindow("createlink",{width:"350px"});
var i=this.currentElement[0],j="",f="",h="",k=false;
if(i){g.el=i;
if(i.getAttribute("href",2)!==null){j=i.getAttribute("href",2);
if(this._isLocalFile(j)){g.setFooter(this.STR_LOCAL_FILE_WARNING);
k=true
}else{g.setFooter(" ")
}}if(i.getAttribute("title")!==null){f=i.getAttribute("title")
}if(i.getAttribute("target")!==null){h=i.getAttribute("target")
}}var l=null;
if(this._windows.createlink&&this._windows.createlink.body){l=this._windows.createlink.body
}else{l=this._renderCreateLinkWindow()
}g.setHeader(this.STR_LINK_PROP_TITLE);
g.setBody(l);
a.purgeElement(this.get("id")+"_createlink_url");
c.get(this.get("id")+"_createlink_url").value=j;
c.get(this.get("id")+"_createlink_title").value=f;
c.get(this.get("id")+"_createlink_target").checked=((h)?true:false);
...
if(this._isLocalFile(j)){c.addClass(this.get("id")+"_createlink_url","warning");
...
},_handleCreateLinkWindowClose:function(){var i=c.get(this.get("id")+"_createlink_url"),g=c.get(this.get("id")+"_createlink_target"),e=c.get(this.get("id")+"_createlink_title"),h=arguments[0].win.el,k=h;
if(i&&i.value){var f=i.value;
if((f.indexOf("://")==-1)&&(f.substring(0,1)!="/")&&(f.substring(0,6).toLowerCase()!="mailto")){if((f.indexOf("@")!=-1)&&(f.substring(0,6).toLowerCase()!="mailto")){f="mailto:"+f
}else{if(f.substring(0,1)!="#"){f="http://"+f
}}}h.setAttribute("href",f);
if(g.checked){h.setAttribute("target",g.value)
}else{h.setAttribute("target","")
}h.setAttribute("title",((e.value)?e.value:""))
}else{var j=this._getDoc().createElement("span");
j.innerHTML=h.innerHTML;
c.addClass(j,"yui-non");
...
this.nodeChange();
this.currentElement=[]
},_renderInsertImageWindow:function(){var l=this.currentElement[0];
var f='<label for="'+this.get("id")+'_insertimage_url"><strong>'+this.STR_IMAGE_URL+':</strong> <input type="text" id="'+this.get("id")+'_insertimage_url" value="" size="40"></label>';
var h=document.createElement("div");
h.innerHTML=f;
var i=document.createElement("div");
i.id=this.get("id")+"_img_toolbar";
h.appendChild(i);
var j='<label for="'+this.get("id")+'_insertimage_title"><strong>'+this.STR_IMAGE_TITLE+':</strong> <input type="text" id="'+this.get("id")+'_insertimage_title" value="" size="40"></label>';
j+='<label for="'+this.get("id")+'_insertimage_link"><strong>'+this.STR_LINK_URL+':</strong> <input type="text" name="'+this.get("id")+'_insertimage_link" id="'+this.get("id")+'_insertimage_link" value=""></label>';
j+='<label for="'+this.get("id")+'_insertimage_target"><strong>&nbsp;</strong><input type="checkbox" name="'+this.get("id")+'_insertimage_target_" id="'+this.get("id")+'_insertimage_target" value="_blank" class="insertimage_target"> '+this.STR_LINK_NEW_WINDOW+"</label>";
var n=document.createElement("div");
n.innerHTML=j;
h.appendChild(n);
var m={};
b.augmentObject(m,this._defaultImageToolbarConfig);
var k=new YAHOO.widget.Toolbar(i,m);
k.editor_el=l;
this._defaultImageToolbar=k;
var e=k.get("cont");
var g=document.createElement("div");
g.className="yui-toolbar-group yui-toolbar-group-height-width height-width";
g.innerHTML="<h3>"+this.STR_IMAGE_SIZE+":</h3>";
g.innerHTML+='<span tabIndex="-1"><input type="text" size="3" value="" id="'+this.get("id")+'_insertimage_width"> x <input type="text" size="3" value="" id="'+this.get("id")+'_insertimage_height"></span>';
...
k.on("colorPickerClicked",function(o){var s="1",p="solid",t="black",q=this._defaultImageToolbar.editor_el;
if(q.style.borderLeftWidth){s=parseInt(q.style.borderLeftWidth,10)
}if(q.style.borderLeftStyle){p=q.style.borderLeftStyle
}if(q.style.borderLeftColor){t=q.style.borderLeftColor
}var r=s+"px "+p+" #"+o.color;
q.style.border=r
},this,true);
k.on("buttonClick",function(o){var q=o.button.value,r=this._defaultImageToolbar.editor_el,s="";
if(o.button.menucmd){q=o.button.menucmd
}var u="1",t="solid",v="black";
if(r.style.borderLeftWidth){u=parseInt(r.style.borderLeftWidth,10)
}if(r.style.borderLeftStyle){t=r.style.borderLeftStyle
}if(r.style.borderLeftColor){v=r.style.borderLeftColor
}switch(q){case"bordersize":if(this.browser.webkit&&this._lastImage){c.removeClass(this._lastImage,"selected");
this._lastImage=null
}s=parseInt(o.button.value,10)+"px "+t+" "+v;
r.style.border=s;
if(parseInt(o.button.value,10)>0){k.enableButton("bordertype");
k.enableButton("bordercolor")
}else{k.disableButton("bordertype");
k.disableButton("bordercolor")
}break;
case"bordertype":if(this.browser.webkit&&this._lastImage){c.removeClass(this._lastImage,"selected");
this._lastImage=null
}s=u+"px "+o.button.value+" "+v;
r.style.border=s;
break;
case"right":case"left":k.deselectAllButtons();
r.style.display="";
r.align=o.button.value;
break;
case"inline":k.deselectAllButtons();
r.style.display="";
r.align="";
break;
case"block":k.deselectAllButtons();
r.style.display="block";
r.align="center";
break;
case"padding":var p=k.getButtonById(o.button.id);
r.style.margin=p.get("label")+"px";
break
}k.selectButton(o.button.value);
if(q!=="padding"){this.moveWindow()
}},this,true);
...
}a.on(this.get("id")+"_insertimage_url","blur",function(){var p=c.get(this.get("id")+"_insertimage_url");
if(p.value&&l){if(p.value==l.getAttribute("src",2)){return false
}}if(this._isLocalFile(p.value)){c.addClass(p,"warning");
this.get("panel").setFooter(this.STR_LOCAL_FILE_WARNING)
}else{if(this.currentElement[0]){c.removeClass(p,"warning");
this.get("panel").setFooter(" ");
if((this.browser.webkit&&!this.browser.webkit3||this.browser.air)||this.browser.opera){this.get("panel").setFooter(this.STR_IMAGE_COPY)
}if(p&&p.value&&(p.value!=this.STR_IMAGE_HERE)){this.currentElement[0].setAttribute("src",p.value);
var q=this,r=new Image();
r.onerror=function(){p.value=q.STR_IMAGE_HERE;
r.setAttribute("src",q.get("blankimage"));
q.currentElement[0].setAttribute("src",q.get("blankimage"));
YAHOO.util.Dom.get(q.get("id")+"_insertimage_height").value=r.height;
YAHOO.util.Dom.get(q.get("id")+"_insertimage_width").value=r.width
};
var o=this.get("id");
window.setTimeout(function(){YAHOO.util.Dom.get(o+"_insertimage_height").value=r.height;
YAHOO.util.Dom.get(o+"_insertimage_width").value=r.width;
if(q.currentElement&&q.currentElement[0]){if(!q.currentElement[0]._height){q.currentElement[0]._height=r.height
}if(!q.currentElement[0]._width){q.currentElement[0]._width=r.width
}}},800);
if(p.value!=this.STR_IMAGE_HERE){r.src=p.value
}}}}},this,true);
this._windows.insertimage={};
this._windows.insertimage.body=h;
h.style.display="none";
this.get("panel").editor_form.appendChild(h);
this.fireEvent("windowInsertImageRender",{type:"windowInsertImageRender",panel:this.get("panel"),body:h,toolbar:k});
return h
},_handleInsertImageClick:function(){if(this.get("limitCommands")){if(!this.toolbar.getButtonByValue("insertimage")){return false
}}this.on("afterExecCommand",function(){var w=this.currentElement[0],o=null,r="",B="",x=null,A="",s="",f="",l=75,h=75,m=0,q=0,t=0,k=false,u=new YAHOO.widget.EditorWindow("insertimage",{width:"415px"});
if(!w){w=this._getSelectedElement()
}if(w){u.el=w;
if(w.getAttribute("src")){s=w.getAttribute("src",2);
if(s.indexOf(this.get("blankimage"))!=-1){s=this.STR_IMAGE_HERE;
k=true
}}if(w.getAttribute("alt",2)){A=w.getAttribute("alt",2)
}if(w.getAttribute("title",2)){A=w.getAttribute("title",2)
}if(w.parentNode&&this._isElement(w.parentNode,"a")){r=w.parentNode.getAttribute("href",2);
if(w.parentNode.getAttribute("target")!==null){B=w.parentNode.getAttribute("target")
}}l=parseInt(w.height,10);
h=parseInt(w.width,10);
if(w.style.height){l=parseInt(w.style.height,10)
}if(w.style.width){h=parseInt(w.style.width,10)
}if(w.style.margin){m=parseInt(w.style.margin,10)
}if(!w._height){w._height=l
}if(!w._width){w._width=h
}q=w._height;
t=w._width
}if(this._windows.insertimage&&this._windows.insertimage.body){o=this._windows.insertimage.body;
this._defaultImageToolbar.resetAllButtons()
}else{o=this._renderInsertImageWindow()
}x=this._defaultImageToolbar;
x.editor_el=w;
var y="0";
var i="solid";
if(w.style.borderLeftWidth){y=parseInt(w.style.borderLeftWidth,10)
}if(w.style.borderLeftStyle){i=w.style.borderLeftStyle
}var e=x.getButtonByValue("bordersize");
var g=((parseInt(y,10)>0)?"":"none");
e.set("label",'<span class="yui-toolbar-bordersize-'+y+'">'+g+"</span>");
this._updateMenuChecked("bordersize",y,x);
var p=x.getButtonByValue("bordertype");
p.set("label",'<span class="yui-toolbar-bordertype-'+i+'"></span>');
this._updateMenuChecked("bordertype",i,x);
if(parseInt(y,10)>0){x.enableButton(p);
x.enableButton(e);
x.enableButton("bordercolor")
}if((w.align=="right")||(w.align=="left")){x.selectButton(w.align)
}else{if(w.style.display=="block"){x.selectButton("block")
}else{x.selectButton("inline")
}}if(parseInt(w.style.marginLeft,10)>0){x.getButtonByValue("padding").set("label",""+parseInt(w.style.marginLeft,10))
}if(w.style.borderSize){x.selectButton("bordersize");
x.selectButton(parseInt(w.style.borderSize,10))
}x.getButtonByValue("padding").set("label",""+m);
u.setHeader(this.STR_IMAGE_PROP_TITLE);
u.setBody(o);
if((this.browser.webkit&&!this.browser.webkit3||this.browser.air)||this.browser.opera){u.setFooter(this.STR_IMAGE_COPY)
}this.openWindow(u);
c.get(this.get("id")+"_insertimage_url").value=s;
c.get(this.get("id")+"_insertimage_title").value=A;
c.get(this.get("id")+"_insertimage_link").value=r;
c.get(this.get("id")+"_insertimage_target").checked=((B)?true:false);
c.get(this.get("id")+"_insertimage_width").value=h;
c.get(this.get("id")+"_insertimage_height").value=l;
var v="";
if((l!=q)||(h!=t)){var n=document.createElement("span");
n.className="info";
n.innerHTML=this.STR_IMAGE_ORIG_SIZE+": ("+t+" x "+q+")";
if(c.get(this.get("id")+"_insertimage_height").nextSibling){var z=c.get(this.get("id")+"_insertimage_height").nextSibling;
z.parentNode.removeChild(z)
}c.get(this.get("id")+"_insertimage_height").parentNode.appendChild(n)
}this.toolbar.selectButton("insertimage");
var j=this.get("id");
window.setTimeout(function(){try{YAHOO.util.Dom.get(j+"_insertimage_url").focus();
if(k){YAHOO.util.Dom.get(j+"_insertimage_url").select()
}}catch(C){}},50)
})
},_handleInsertImageWindowClose:function(){var l=c.get(this.get("id")+"_insertimage_url"),e=c.get(this.get("id")+"_insertimage_title"),h=c.get(this.get("id")+"_insertimage_link"),g=c.get(this.get("id")+"_insertimage_target"),i=arguments[0].win.el;
if(l&&l.value&&(l.value!=this.STR_IMAGE_HERE)){i.setAttribute("src",l.value);
i.setAttribute("title",e.value);
i.setAttribute("alt",e.value);
var j=i.parentNode;
if(h.value){var f=h.value;
if((f.indexOf("://")==-1)&&(f.substring(0,1)!="/")&&(f.substring(0,6).toLowerCase()!="mailto")){if((f.indexOf("@")!=-1)&&(f.substring(0,6).toLowerCase()!="mailto")){f="mailto:"+f
}else{f="http://"+f
}}if(j&&this._isElement(j,"a")){j.setAttribute("href",f);
if(g.checked){j.setAttribute("target",g.value)
}else{j.setAttribute("target","")
}}else{var k=this._getDoc().createElement("a");
k.setAttribute("href",f);
if(g.checked){k.setAttribute("target",g.value)
}else{k.setAttribute("target","")
}i.parentNode.replaceChild(k,i);
k.appendChild(i)
}}else{if(j&&this._isElement(j,"a")){j.parentNode.replaceChild(i,j)
...
this.currentElement=[];
this.nodeChange()
},EDITOR_PANEL_ID:"-panel",_renderPanel:function(){var m=new YAHOO.widget.Overlay(this.get("id")+this.EDITOR_PANEL_ID,{width:"300px",iframe:true,visible:false,underlay:"none",draggable:false,close:false});
this.set("panel",m);
this.get("panel").setBody("---");
this.get("panel").setHeader(" ");
this.get("panel").setFooter(" ");
var h=document.createElement("div");
h.className=this.CLASS_PREFIX+"-body-cont";
for(var g in this.browser){if(this.browser[g]){c.addClass(h,g);
break
}}c.addClass(h,((YAHOO.widget.Button&&(this._defaultToolbar.buttonType=="advanced"))?"good-button":"no-button"));
var j=document.createElement("h3");
j.className="yui-editor-skipheader";
j.innerHTML=this.STR_CLOSE_WINDOW_NOTE;
h.appendChild(j);
var l=document.createElement("form");
l.setAttribute("method","GET");
m.editor_form=l;
a.on(l,"submit",function(n){a.stopEvent(n)
},this,true);
h.appendChild(l);
var k=document.createElement("span");
k.innerHTML="X";
k.title=this.STR_CLOSE_WINDOW;
k.className="close";
a.on(k,"click",this.closeWindow,this,true);
...
m.setFooter(" ");
m.appendToBody(h);
a.on(m.element,"click",function(n){a.stopPropagation(n)
});
var i=function(){};
m.showEvent.subscribe(i,this,true);
m.renderEvent.subscribe(function(){this._renderInsertImageWindow();
this._renderCreateLinkWindow();
this.fireEvent("windowRender",{type:"windowRender",panel:m})
},this,true);
...
},openWindow:function(k){var f=this;
window.setTimeout(function(){f.toolbar.set("disabled",true)
},10);
a.on(document,"keydown",this._closeWindow,this,true);
if(this.currentWindow){this.closeWindow()
}var e=c.getXY(this.currentElement[0]),h=c.getXY(this.get("iframe").get("element")),q=this.get("panel"),n=[(e[0]+h[0]-20),(e[1]+h[1]+10)],o=(parseInt(k.attrs.width,10)/2),j="center",l=null;
this.fireEvent("beforeOpenWindow",{type:"beforeOpenWindow",win:k,panel:q});
var p=q.editor_form;
var m=this._windows;
for(var g in m){if(b.hasOwnProperty(m,g)){if(m[g]&&m[g].body){if(g==k.name){c.setStyle(m[g].body,"display","block")
}else{c.setStyle(m[g].body,"display","none")
}}}}if(this._windows[k.name].body){c.setStyle(this._windows[k.name].body,"display","block");
p.appendChild(this._windows[k.name].body)
}else{if(b.isObject(k.body)){p.appendChild(k.body)
}else{var i=document.createElement("div");
i.innerHTML=k.body;
p.appendChild(i)
}}q.editor_header.firstChild.innerHTML=k.header;
if(k.footer!==null){q.setFooter(k.footer);
c.addClass(q.footer,"open")
}else{c.removeClass(q.footer,"open")
}q.cfg.setProperty("width",k.attrs.width);
this.currentWindow=k;
this.moveWindow(true);
q.show();
this.fireEvent("afterOpenWindow",{type:"afterOpenWindow",win:k,panel:q})
},moveWindow:function(y){if(!this.currentWindow){return false
}var v=this.currentWindow,u=c.getXY(this.currentElement[0]),B=c.getXY(this.get("iframe").get("element")),p=this.get("panel"),f=[(u[0]+B[0]),(u[1]+B[1])],m=(parseInt(v.attrs.width,10)/2),j="center",n=p.cfg.getProperty("xy")||[0,0],x=p.editor_knob,g=0,s=0,k=false;
f[0]=((f[0]-m)+20);
f[0]=f[0]-c.getDocumentScrollLeft(this._getDoc());
f[1]=f[1]-c.getDocumentScrollTop(this._getDoc());
if(this._isElement(this.currentElement[0],"img")){if(this.currentElement[0].src.indexOf(this.get("blankimage"))!=-1){f[0]=(f[0]+(75/2));
f[1]=(f[1]+75)
}else{var q=parseInt(this.currentElement[0].width,10);
var h=parseInt(this.currentElement[0].height,10);
f[0]=(f[0]+(q/2));
f[1]=(f[1]+h)
}f[1]=f[1]+15
}else{var t=c.getStyle(this.currentElement[0],"fontSize");
if(t&&t.indexOf&&t.indexOf("px")!=-1){f[1]=f[1]+parseInt(c.getStyle(this.currentElement[0],"fontSize"),10)+5
}else{f[1]=f[1]+20
}}if(f[0]<B[0]){f[0]=B[0]+5;
j="left"
}if((f[0]+(m*2))>(B[0]+parseInt(this.get("iframe").get("element").clientWidth,10))){f[0]=((B[0]+parseInt(this.get("iframe").get("element").clientWidth,10))-(m*2)-5);
j="right"
}try{g=(f[0]-n[0]);
s=(f[1]-n[1])
}catch(A){}if(this.get("autoHeight")===false){var o=B[1]+parseInt(this.get("height"),10);
var w=B[0]+parseInt(this.get("width"),10);
if(f[1]>o){f[1]=o
}if(f[0]>w){f[0]=(w/2)
}}g=((g<0)?(g*-1):g);
s=((s<0)?(s*-1):s);
if(((g>10)||(s>10))||y){var l=0,i=0;
if(this.currentElement[0].width){i=(parseInt(this.currentElement[0].width,10)/2)
}var r=u[0]+B[0]+i;
l=r-f[0];
if(l>(parseInt(v.attrs.width,10)-1)){l=((parseInt(v.attrs.width,10)-30)-1)
}else{if(l<40){l=1
}}if(isNaN(l)){l=1
}if(y){if(x){x.style.left=l+"px"
}p.cfg.setProperty("xy",f)
}else{if(this.get("animate")){k=new YAHOO.util.Anim(p.element,{},0.5,YAHOO.util.Easing.easeOut);
k.attributes={top:{to:f[1]},left:{to:f[0]}};
k.onComplete.subscribe(function(){p.cfg.setProperty("xy",f)
});
var e=new YAHOO.util.Anim(p.iframe,k.attributes,0.5,YAHOO.util.Easing.easeOut);
var z=new YAHOO.util.Anim(x,{left:{to:l}},0.6,YAHOO.util.Easing.easeOut);
k.animate();
e.animate();
z.animate()
}else{x.style.left=l+"px";
p.cfg.setProperty("xy",f)
}}}},_closeWindow:function(e){if(this._checkKey(this._keyMap.CLOSE_WINDOW,e)){if(this.currentWindow){this.closeWindow()
}}},closeWindow:function(e){this.fireEvent("window"+this.currentWindow.name+"Close",{type:"window"+this.currentWindow.name+"Close",win:this.currentWindow,el:this.currentElement[0]});
...
},cmd_undo:function(e){if(this._hasUndoLevel()){if(!this._undoLevel){this._undoLevel=this._undoCache.length
}this._undoLevel=(this._undoLevel-1);
if(this._undoCache[this._undoLevel]){var f=this._getUndo(this._undoLevel);
this.setEditorHTML(f)
}else{this._undoLevel=null;
this.toolbar.disableButton("undo")
}}return[false]
},cmd_redo:function(e){this._undoLevel=this._undoLevel+1;
if(this._undoLevel>=this._undoCache.length){this._undoLevel=this._undoCache.length
}if(this._undoCache[this._undoLevel]){var f=this._getUndo(this._undoLevel);
this.setEditorHTML(f)
}else{this.toolbar.disableButton("redo")
}return[false]
},cmd_heading:function(f){var i=true,h=null,g="heading",e=this._getSelection(),j=this._getSelectedElement();
if(j){e=j
}if(this.browser.ie){g="formatblock"
}if(f=="none"){if((e&&e.tagName&&(e.tagName.toLowerCase().substring(0,1)=="h"))||(e&&e.parentNode&&e.parentNode.tagName&&(e.parentNode.tagName.toLowerCase().substring(0,1)=="h"))){if(e.parentNode.tagName.toLowerCase().substring(0,1)=="h"){e=e.parentNode
}if(this._isElement(e,"html")){return[false]
}h=this._swapEl(j,"span",function(k){k.className="yui-non"
});
this._selectNode(h);
this.currentElement[0]=h
}i=false
}else{if(this._isElement(j,"h1")||this._isElement(j,"h2")||this._isElement(j,"h3")||this._isElement(j,"h4")||this._isElement(j,"h5")||this._isElement(j,"h6")){h=this._swapEl(j,f);
this._selectNode(h);
this.currentElement[0]=h
...
}else{this._showingHiddenElements=true;
c.addClass(this._getDoc().body,this.CLASS_HIDDEN);
this.toolbar.selectButton("hiddenelements")
}return[false]
},cmd_removeformat:function(e){var g=true;
if(this.browser.webkit&&!this._getDoc().queryCommandEnabled("removeformat")){var h=this._getSelection()+"";
this._createCurrentElement("span");
this.currentElement[0].className="yui-non";
this.currentElement[0].innerHTML=h;
for(var f=1;
f<this.currentElement.length;
f++){this.currentElement[f].parentNode.removeChild(this.currentElement[f])
}g=false
}return[g]
},cmd_script:function(e,f){var i=true,k=e.toLowerCase().substring(0,3),h=null,j=this._getSelectedElement();
if(this.browser.webkit){if(this._isElement(j,k)){h=this._swapEl(this.currentElement[0],"span",function(l){l.className="yui-non"
});
this._selectNode(h)
}else{this._createCurrentElement(k);
var g=this._swapEl(this.currentElement[0],k);
this._selectNode(g);
this.currentElement[0]=g
}i=false
}return i
...
},cmd_indent:function(f){var i=true,g=this._getSelectedElement(),e=null;
if(this.browser.ie){if(this._isElement(g,"blockquote")){e=this._getDoc().createElement("blockquote");
e.innerHTML=g.innerHTML;
g.innerHTML="";
g.appendChild(e);
this._selectNode(e)
}else{e=this._getDoc().createElement("blockquote");
var h=this._getRange().htmlText;
e.innerHTML=h;
this._createCurrentElement("blockquote");
this.currentElement[0].parentNode.replaceChild(e,this.currentElement[0]);
this.currentElement[0]=e;
this._selectNode(this.currentElement[0])
}i=false
}else{f="blockquote"
}return[i,"formatblock",f]
},cmd_outdent:function(f){var j=true,g=this._getSelectedElement(),e=null,i=null;
if(this.browser.webkit||this.browser.ie){g=this._getSelectedElement();
if(this._isElement(g,"blockquote")){var h=g.parentNode;
if(this._isElement(g.parentNode,"blockquote")){h.innerHTML=g.innerHTML;
this._selectNode(h)
}else{i=this._getDoc().createElement("span");
i.innerHTML=g.innerHTML;
YAHOO.util.Dom.addClass(i,"yui-non");
h.replaceChild(i,g);
this._selectNode(i)
}}else{}j=false
}else{f=false
...
YAHOO.widget.EditorWindow=function(e,f){this.name=e.replace(" ","_");
this.attrs=f
};
